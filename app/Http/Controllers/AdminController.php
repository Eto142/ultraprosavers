...
public function ManageUsers()
{
   ...
   $result = User::all();
    ...
    public function totalLoans()
    {
        ...
        $totalAmount = Loan::where('status', '1')->sum('amount');
        $totalCount = Loan::count();
        $pendingCount = Loan::where('status', '0')->count();
        $approvedCount = Loan::where('status', '1')->count();
        $declinedCount = Loan::where('status', '2')->count();
        return view('admin.total_loans', compact('loans', 'totalAmount', 'totalCount', 'pendingCount', 'approvedCount', 'declinedCount'));
    }

    public function totalDeposits()
    {
        $deposits = Deposit::with('user')->orderBy('id', 'desc')->get();
        $totalAmount = Deposit::where('status', '1')->sum('amount');
        $totalCount = Deposit::count();
        $pendingCount = Deposit::where('status', '0')->count();
        $approvedCount = Deposit::where('status', '1')->count();
        $declinedCount = Deposit::where('status', '2')->count();
        return view('admin.total_deposits', compact('deposits', 'totalAmount', 'totalCount', 'pendingCount', 'approvedCount', 'declinedCount'));
    }
...
public function iUserBlock($id)
{
    ...
    return back()->with('status', 'Account UnBlocked!');  
...
public function updateEmail(Request $request, $id)
{
    ...
    return back()->with('status', 'user Email Updated Successfully');  
...
public function DeclineTransaction(Request $request, $id)
{
    ...
    $email = $transaction->email; // Assuming there's a user relationship in the transactions table
    $amount = $transaction->transaction_amount;
    $currency = $request->currency; // Make sure this is coming from the request
    $transactionType = $transaction->transaction; // Assuming this is the correct property name
    $transactionId = $transaction->transaction_id; // Assuming this is the correct property name

    // Compose the email message
    $data = "Your " . $currency . " " . number_format($amount, 2) . " for " . $transactionType . " with the transaction ID " . $transactionId . " has been declined.";
